...
//this is the issue that will be created that we want the CSV to be attached to

def createTicket = post('/rest/api/2/issue')
        .header('Content-Type', 'application/json')
        .body(
                [
                        fields: [
        ...
        assert createTicket.status >= 200 && createTicket.status <= 300


        def issueKey = createTicket.body.key.toString()
